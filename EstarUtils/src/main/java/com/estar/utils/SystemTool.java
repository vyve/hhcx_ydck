...
//    }
    ...
    public static String getAppVersion(Context context) {
        String versionName = "0";//版本名称
        String versionCode = "0";//版本大小
        try {
     ...
     * @return
     */
    public static File ZMdownloadTheFile(final String strPath) {
        try {
            String fileEx = strPath.substring(strPath.lastIndexOf(".") + 1)
                    .toLowerCase();   //获取后缀名
            String fileNa = strPath.substring(strPath.lastIndexOf("/") + 1,
                    strPath.lastIndexOf("."));    //获取文件名
            String path = Environment.getExternalStorageDirectory()+"/download/ZMversion"; //配置下载路径
            String fileName = fileNa + "." +fileEx; //文件名
            File file = new File(path);
//        deleteDir(file);
            file.mkdirs();

            return new File(path+"/"+fileName); //创建文件
        } catch (Exception e) {
            e.printStackTrace();
            return null; //创建文件
        }
     ...
     * @return
     */
    public static File JYdownloadTheFile(final String strPath) {
        try {
            String fileEx = strPath.substring(strPath.lastIndexOf(".") + 1)
                    .toLowerCase();   //获取后缀名
            String fileNa = strPath.substring(strPath.lastIndexOf("/") + 1,
...
//        ToastUtils.showShort(cxt,"清理了" + (getDeviceUsableMemory(cxt) - i) + "M内存");
...
//                + "M内存");
     ...
     * @param cxt
     */
    public static void getRunningAppProcessInfo(Context cxt) {
        ActivityManager mActivityManager = (ActivityManager) cxt.getSystemService(Context.ACTIVITY_SERVICE);
...
//            LogUtil.w("processName="+processName+",pid="+pid+",uid="+uid+",memorySize="+memorySize+"kb");
        }
    }
}